...
class Solution {
    public boolean canIWin(int maxChoosableInteger, int desiredTotal) {
        ...
        return canIWinCore(maxChoosableInteger, desiredTotal, map, 0);
    }
    
    private boolean canIWinCore(int max, int target, Map<Integer, Boolean> map, int visited) {
        ...
        for (int i = 1; i <= max; i++) {
            int mask = (1 << i);
            if ((visited & mask) == 0 && (i >= target || (canIWinCore(max, target - i, map, visited | mask) == false))) {
